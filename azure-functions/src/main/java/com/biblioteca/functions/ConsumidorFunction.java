...
/**
 * Consumidor de eventos de Azure Event Grid.
 *
 * Maneja dos tipos de evento:
 *  - "Prestamo.Creado"   → UPDATE libros SET disponibilidad = disponibilidad - 1 WHERE titulo = ?
 *  - "Usuario.Eliminado" → DELETE FROM prestamos WHERE id_usuario = ?
 *                           DELETE FROM usuarios  WHERE id_usuario = ?  (en una sola transacción)
 */
public class ConsumidorFunction {
    ...
    public void run(
        ...
        try {
            // El mensaje de Event Grid es un JSON con "eventType" y "data"
     ...
     * Payload esperado: { "tituloLibro": "...", "idUsuario": N }
     *
     * Ejecuta:  UPDATE libros SET disponibilidad = disponibilidad - 1
     *           WHERE titulo = ?
     */
    private void manejarPrestamoCreado(JsonObject data, ExecutionContext context) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            throw new RuntimeException(e); // Propaga para que Azure Functions reintente si está configurado
        }
    }
     ...
     * Payload esperado: { "idUsuario": N }
     *
     * Ejecuta en una ÚNICA transacción (autoCommit = false):
     *   1. DELETE FROM prestamos WHERE id_usuario = ?
     *   2. DELETE FROM usuarios  WHERE id_usuario = ?
     */
    private void manejarUsuarioEliminado(JsonObject data, ExecutionContext context) {
        int idUsuario = data.get("idUsuario").getAsInt();
        context.getLogger().info(
            "ConsumidorFunction [Usuario.Eliminado] – Iniciando eliminación en cascada para id_usuario: " + idUsuario);
        ...
        try {
            ...
            // Paso 1: Eliminar los préstamos del usuario (respeta la FK)
            ...
            // Paso 2: Eliminar al usuario (ahora sin préstamos referenciándolo)
            try (PreparedStatement stmtUsuario = conn.prepareStatement(
                ...
                if (usuariosBorrados > 0) {
                    context.getLogger().info(
                        "ConsumidorFunction [Usuario.Eliminado] – Usuario " + idUsuario + " eliminado correctamente.");
                } else {
                    context.getLogger().warning(
            ...
            context.getLogger().info(
                "ConsumidorFunction [Usuario.Eliminado] – Transacción COMMIT exitosa para id_usuario: " + idUsuario);
